...
impl Config {
    fn from(opt_matches: &getopts::Matches) -> Result<Self, ()> {
        ...
        Ok(Config {
            input_file,
            bullet: get_opt_or_default!("b", bullet),
            indent: get_opt_or_default!("indent", indent),
            max_depth: get_opt_or_default!("max-depth", max_depth),
            min_depth: get_opt_or_default_option!("min-depth", min_depth),
...
fn parse_command(opts: &mut getopts::Options, args: &[String]) -> Result<Config, ()> {
    opts.optflag("h", "help", "print this help message")
        ...
        .optopt(
            "",
            "max-depth",
            "Max depth of headers to include. (default: 0)",
            "{int}",
        )
        .optopt("", "min-depth", "Min depth of headers to include.", "{int}")
        .optopt(
            "h",
            "header",
...
fn main() {
    ...
    match config.input_file {
        ...
        InputFile::Path(p) => File::open(p).unwrap().read_to_string(&mut content),
    }.unwrap();
